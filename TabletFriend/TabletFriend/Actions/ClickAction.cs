...
namespace TabletFriend.Actions
{
	public class ClickAction : ButtonAction
	{
		...
		public async override Task Invoke()
		{
			if (_initialized)
			{
				await Simulate.Events().MoveTo(_x, _y).Click(ButtonCode.Left).Invoke();
			}
		}
		...
		private static void MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
		{
			if (e.LeftButton == System.Windows.Input.MouseButtonState.Pressed)
			{
					...
					!_dragBoxFromMouseDown.Contains(x, y))
				{
					_dragBoxFromMouseDown = Rectangle.Empty;
					{
						DataObject dataObj = new DataObject(sender as ButtonBase);
						System.Windows.DragDrop.DoDragDrop(sender as ButtonBase, dataObj, DragDropEffects.None);
						if (_newClickCoordinates.HasValue)
						{
							...
							{
								var key = (sender as ButtonBase).Name;
